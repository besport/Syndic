...
(** An [image] is an optional sub-element of {!channel}, which
    contains three required ([url], [title], [link]) and three
    optional ([width], [height], [description]) sub-elements.
...
(** [category] is an optional sub-element of {!item}.
    ...
    - [domain], if provided, a string that identifies a categorization
      taxonomy.
...
(** [guid] is an optional sub-element of {!item}.  "guid" stands for
    ...
    use this string to determine if an item is new.
    ...
    If [permalink] is [true], the reader may assume that it is a
    permalink to the item, that is, a url that can be opened in a Web
    browser, that points to the full item described by the <item>
    element. An example:

    {[<guid isPermaLink="true">http://inessential.com/2002/09/01.php#a2</guid>]}

    If [permalink] is [false], the guid may not be assumed to be a
...
(** Here's a list of the required channel elements, each with a brief
    ...
    - [language]: The language the channel is written in.  This allows
      aggregators to group all Italian language sites, for example, on
      a single page.  A list of allowable values for this element, as
      provided by Netscape, is here.  You may also use values defined
      by the W3C.
    - [copyright]: Copyright notice for content in the channel.
    - [managingEditor]: Email address for person responsible for
      editorial content.
    ...
    - [pubDate]: The publication date for the content in the channel.
      ...
      pubDate of the channel changes.  All date-times in RSS conform
      to the Date and Time Specification of RFC 822, with the
      exception that the year may be expressed with two characters or
    ...
    - [category]: Specify one or more categories that the channel
      belongs to.  Follows the same rules as the <item>-level category
    ...
    - [cloud]: Allows processes to register with a cloud to be
      notified of updates to the channel, implementing a lightweight
      publish-subscribe protocol for RSS feeds.  See {!cloud}.
    - [ttl]: ttl stands for time to live.
      It's a number of minutes that indicates how long a channel can
      be cached before refreshing from the source.
    - [image]:
      Specifies a GIF, JPEG or PNG image that can be displayed
      with the channel.  See {!image}.
    - [rating]: The PICS rating for the channel.
    - [textInput]: Specifies a text input box that can be displayed
      with the channel.  See {!textinput}.
    - [skipHours]: A hint for aggregators telling them which hours they
      can skip.  This element contains up to 24 <hour> sub-elements
      ...
      the channel on hours listed in the <skipHours> element.  The
      hour beginning at midnight is hour zero.
    - [skipDays]: A hint for aggregators telling them which days they
      can skip.  This element contains up to seven <day> sub-elements
